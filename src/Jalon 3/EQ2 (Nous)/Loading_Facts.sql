...
COPY Fact_Humidite(id_interne_zone, date, hum_min, hum_max, temp_moyenne, variation, note) FROM '/EQ2/Loading/Fact_Humidite.csv' WITH (FORMAT CSV, HEADER);

COPY Fact_Vents(id_interne_zone, date, vent_min, vent_max, temp_moyenne, variation, note) FROM '/EQ2/Loading/Fact_Vents.csv' WITH (FORMAT CSV, HEADER);

COPY Fact_Pression(id_interne_zone, date, pres_min, pres_max, temp_moyenne, variation, note) FROM '/EQ2/Loading/Fact_Pression.csv' WITH (FORMAT CSV, HEADER);
